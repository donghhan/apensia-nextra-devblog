...
Github에서 제공하고 있는 Github Page에 자동으로 배포하는 태스크를 붙여보겠다.
...
<a href="https://github.com/shinnn/gulp-gh-pages#readme" target="_blank">gulp-gh-pages</a>: Github page에 자동으로 배포까지 해주는 편리한 플러그인이다.

<CH.Scrollycoding>

### Importing

필요한 package를 `import` 해준다.
...
이 때, 우리가 최종적으로 Github Pages에 업로드할 파일들은 이미 빌드가 끝마친 파일들이므로 `.dest()`를 `build/` 폴더를 바라보게끔 해준다.[^3]

[^3]: 참조: <a href="https://github.com/shinnn/gulp-gh-pages#usage" target="_blank">gulp-gh-pages Usage</a>
...
따로 `deploy`라는 스크립트 명령어를 추가해줬다.
이 명령어는 앞서 정의한 `progress` 명령어를 실행한 후, `GhPages()` 태스크를 수행하여 Github Pages에 배포하는 작업을 시행한다.

여기서 `Clean()` 태스크를 따로 정의해준 이유는 `GhPages()` 태스크가 Github Pages에 배포하면서 만들어내는 더미 파일들을 없애기 위함이다.
...
마지막으로 만든 `deploy` 명령어를 `package.json` 파일에 업데이트 시켜준다.
...
`npm run deploy` 혹은 `yarn deploy` 명령어를 실행시키면 Gulp가 자동적으로 `gh-pages`라는 브랜치를 생성해준다.
이 브랜치는 Github Pages에 배포할 시 자동으로 생성되는 브랜치이며, Github Page가 해당 브랜치를 바라볼 수 있도록 설정해주어야 한다.

<center>![deploy_result](../../media/gulp-configuration-2/deploy_result.png)</center>

<br/>

레포지토리의 Settings에서 좌측의 Code and automation의 Pages 탭으로 들어간다.
이후, Source의 Branch를 gh-pages로 설정한 후, /(root)를 바라보게 한 다음 Save 버튼을 누른다.
...
5분 정도 기다린 후 기다리면 자동으로 배포된 모습을 볼 수 있다.
