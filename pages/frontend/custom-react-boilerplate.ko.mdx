...
<Admonition 
    keyword="caution"
    style={{ color: '#c29200', fontSize: '16px' }}
    title={<span><strong>No inputs were found in config file tsconfig.json</strong></span>}
  >
    <center>![error_tsconfig](../../media/frontend/custom-react-boilerplate/error_tsconfig.png)</center>
    해당 에러는 `tsconfig.json` 내부에 `entry` 부분을 지정해주지 않았기 때문에 발생하는 에러이므로, <a href="https://stackoverflow.com/a/41211721/13121145" target="_blank">`include` 내부에 지정된 폴더 안에 TypeScript 파일 아무거나 생성해주면</a> 에러 해결이 가능하다.
