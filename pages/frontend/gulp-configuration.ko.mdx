---
title: Pug, SCSS 및 TypeScript를 위한 Gulp 설정 (1)
...
<a href="https://pugjs.org/api/getting-started.html" target="_blank">Pug</a>는 NodeJS의 템플릿 엔진이다. 
이 Pug를 HTML 파일로 변환시켜보자.
...
1. <a href="https://github.com/gulp-community/gulp-pug#readme" target="_blank">gulp-pug</a>: Pug 파일을 일반 HTML 파일로 변환시켜주는 Gulp의 플러그인이다.
2. <a href="https://github.com/jonschlinkert/gulp-htmlmin" target="_blank">gulp-htmlmin</a>: 성능 향상을 위해 변환된 HTML을 압축해준다.
...
이미지 최적화를 위한 작업을 마지막으로 해볼 것이다. 다음 패키지를 설치해주도록 하자.
...
1. <a href="https://github.com/1000ch/gulp-image#readme" target="_blank">gulp-image</a>: 이미지의 용량을 최적화해주는 Gulp의 플러그인 중 하나이다.

<CH.Scrollycoding>

### 경로

최적화시킬 이미지들은 `src/image/` 폴더에 전부 집어넣었고, 최적화가 끝난 이미지는 `build/image/` 폴더에 전부 집어넣는 것으로 하였다.
...
다음 코드를 삽입하여 이미지를 최적화 작업을 진행해준다.[^3]

[^3]: 참조: <a href="https://github.com/1000ch/gulp-image#usage" target="_blank">gulp-image 패키지 옵션들</a>
...
`dev` 명령어를 실행시키면 아래 그림처럼 Gulp가 자동적으로 이미지 최적화를 진행시켜준다.
