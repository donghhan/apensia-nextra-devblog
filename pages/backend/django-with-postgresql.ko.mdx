...
description: 'Django를 PostgreSQL와 연동하기'
...
<Admonition 
    ...
    title={<span><strong>FATAL: password authentication failed for user "postgres" 에러</strong></span>}
  >
   해당 유저에 대한 비밀번호를 설정해주어야 한다[^3]. 콘솔창에 다음과 같은 명령어를 통해 비밀번호를 설정해준다.

```bash
sudo -u postgres psql -c "ALTER ROLE postgres WITH password '원하는 비밀번호 아무거나'"
```
...
<Admonition 
    keyword="error"
    style={{ color: 'red', fontSize: '16px' }}
    title={<span><strong>django.db.utils.OperationalError: connection to server at "localhost" (127.0.0.1), port 5432 failed: Connection refused 에러</strong></span>}
  >
   에러 글자 그대로에서 알 수 있듯이 서버가 시작되지 않은 상황에서 서버를 실행시키면 나타나는 에러이다[^4].
...
Ubuntu의 경우 다음 콘솔 명령어를 통해서 서버를 시작한다.
