...
description: 'Django에서 다국어 기능을 구현해보자.'
---
...
다국어 지원 기능을 생각한다면 **_프로젝트 맨 처음부터 미리 준비해두는 것이 좋다._**

### 사전 준비

`locale`이라는 폴더 안에 우리가 작업할 외국어 폴더를 집어넣는다.
`settings.py` 파일 안의 <a href="https://docs.djangoproject.com/en/4.1/ref/settings/#language-code" target="_blank"><code>LANGUAGE_CODE</code></a>의 기본값은 `en-us`로 설정되어 있으며,
이 기본값으로 지정된 언어 이외에 추가적으로 지원할 외국어 개수만큼 `locale` 폴더 안에 외국어 폴더를 만들어주어야 한다.

#### `locale` 폴더 생성

`locale`이란 폴더를 프로젝트 루트 경로에 생성해주고, 필자의 경우 태국어를 지원할 것이기 때문에 태국어(`th`) 폴더를 생성해주었다.
...
마지막으로 `settings.py` 파일에 <a href="https://docs.djangoproject.com/en/4.1/ref/settings/#std-setting-USE_I18N" target="_blank"><code>USE_I18N</code></a>에 `True` 값으로 설정해주자.
...
이제 본격적으로 번역 작업을 시작해보도록 하자. 여기서부터 기본으로 설정되어 있는 영어 원본 파일을 우리가 번역하고자 하는 다른 언어 파일로 번역해주는 작업이 되겠다.
...
> `lazy`라는 끝말이 붙었다는 것은 글자 그대로 어떤 작업을 <ins>뒤늦게</ins> 작업을 한다는 뜻이다.
> 즉, 값이 처음 호출되었을 때부터가 아닌 값에 접근했을 때 해당 작업이 이루어진다는 뜻이다.[^1]

[^1]: 참조: <a href="https://nsikakimoh.com/blog/gettext-and-gettext_lazy-functions-in-django#what-is-the-difference-between-gettext-and-gettext_lazy" target="_blank">What is the Difference Between `gettext()` and `gettext_lazy()`</a>
...
<Callout>
    `models.py`나 `forms.py` 및 `apps.py`에 정의된 필드들은 프로젝트가 시작될 때 오직 한 번만 호출이 되기 때문에 `gettext` 모듈을 사용하게 될 경우 맨 처음 설정된 기본 언어로만 번역이 되어 
    추후 유저가 언어를 변경해도 적용이 되질 않게 된다. 이 때문에 `gettext_lazy` 모듈을 사용하도록 한다.[^2]
</Callout>

[^2]: 참조: <a href="https://nsikakimoh.com/blog/gettext-and-gettext_lazy-functions-in-django#why-you-should-use-gettext_lazy-in-models-and-forms-instead-of-gettext" target="_blank">Why you should use `gettext_lazy()` in Models and Forms instead of `gettext()`</a>
