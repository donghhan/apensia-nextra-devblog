...
&rarr; <a href="https://stackpython.medium.com/how-to-start-django-project-with-a-database-postgresql-aaa1d74659d8" target="_blank">How to Start Django Project with a Database(PostgreSQL)</a>
...
<Admonition 
    ...
    title={<span><strong>FATAL: password authentication failed for user "postgres" error</strong></span>}
  >
   You need to setup the password for the user[^3]. Set up the password with the following command on console.

```bash
sudo -u postgres psql -c "ALTER ROLE postgres WITH password 'anyPasswordYouWant'"
```
...
<center>![registering_server](../../media/backend/connecting-postgresql-with-django/registering_server.png)</center>
...
<Admonition 
    keyword="error"
    style={{ color: 'red', fontSize: '16px' }}
    title={<span><strong>django.db.utils.OperationalError: connection to server at "localhost" (127.0.0.1), port 5432 failed: Connection refused</strong></span>}
  >
   As you can literally tell from its error message, this is because you did not start server[^4].
...
In case of Ubuntu, start server with the following command.
...
<center>![registering_server](../../media/backend/connecting-postgresql-with-django/view_data.png)</center>
...
[^3]:
    <a href="https://stackoverflow.com/a/28585705/13121145" target="_blank">psql: FATAL: password authentication failed for user "postgres" postgresql version 10</a>

[^4]:
    <a href="https://stackoverflow.com/a/35367219/13121145" target="_blank">How do I automatically start a service on Ubuntu</a>
