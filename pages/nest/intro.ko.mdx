...
`app.module.ts` 파일은 글자 그대로 NestJS의 app에서 모듈을 담당하는 부분이다.
...
`app.controller.ts` 파일은 [Controller](https://docs.nestjs.com/controllers)이며 주 역할은 <ins>_URL를 가져온 후 함수를 실행시키는 역할_</ins>을 수행한다.
...
[Controller](https://docs.nestjs.com/controllers)는
