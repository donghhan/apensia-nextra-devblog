...
<details>
  ...
  <p>
    교집합을 이용하여 역으로 계산하는 방법이 있었다. 
    집합 B를 `Set()`에 넣은 후 집합 A의 모든 원소에 대하여 집합 B에 집합 A의 원소가 있는지 판별 후 만일 있다면 교집합 개수를 1개씩 늘린다.
    이후 집합 A의 크기인 `N`, 집합 B의 크기인 `M`를 더한 후 교집합 개수값을 빼주면 대칭 차집합이 나오게 된다.

    하지만 이 경우에는 집합 A의 크기가 항상 집합 B의 크기보다 작을 경우에만 해당되기 때문에 로직을 좀 더 손봐야 하지 않을까 싶다.
