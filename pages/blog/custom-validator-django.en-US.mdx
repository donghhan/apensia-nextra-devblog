...
Finally, we are going to check if user's password <a href="https://docs.python.org/3/library/stdtypes.html?highlight=isalpha#bytearray.isalpha"><code>isupper()</code></a> method, we will check all characters whether they are uppercases.
If it is, then it will return `True`.
